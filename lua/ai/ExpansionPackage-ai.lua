...
sgs.ai_skill_use['@@LuaYinghun'] = function(self, prompt, method)
    ...
    if x == 1 then
        ...
        if not target then
            -- 配合邓艾等屯田
            for _, friend in ipairs(self.friends_noself) do
                if (friend:hasSkills('LuaTuntian+LuaZaoxian') or friend:hasSkills('tuntian+zaoxian')) and not friend:hasSkill('manjuan') then
                    target = friend
                    break
                end
            end
        ...
        if not target then
            ...
            for _, enemy in ipairs(self.enemies) do
                if not enemy:isNude() and
                    ...
                    not self:needToThrowArmor(enemy) and not (enemy:hasSkills('LuaTuntian+LuaZaoxian') or enemy:hasSkills('tuntian+zaoxian')) then
                    target = enemy
                    break
                end
            end
            if not target then
                for _, enemy in ipairs(self.enemies) do
                    if not enemy:isNude() and
                        ...
                        not ((enemy:hasSkills('LuaTuntian+LuaZaoxian') or enemy:hasSkills('tuntian+zaoxian')) and x < 3 and enemy:getCards('he'):length() < 2) then
                        target = enemy
                        break
                    end
                end
...
sgs.ai_skill_invoke.LuaTuntian = function(self, data)
	if self.player:hasSkill('LuaZaoxian') and #self.enemies == 1 and self.room:alivePlayerCount() == 2
		and self.player:getMark('LuaZaoxian') == 0 and self:hasSkills('noswuyan|qianxun', self.enemies[1]) then
			return false
	end
	return true
end
...
sgs.ai_slash_prohibit.LuaTuntian = function(self, from, to, card)
	...
	if not to:hasSkill('LuaZaoxian') then return false end
	if from:hasSkill('tieji') or self:canLiegong(to, from) then
		return false
	end
	local enemies = self:getEnemies(to)
	if #enemies == 1 and self.room:alivePlayerCount() == 2 and self:hasSkills('noswuyan|qianxun|weimu|LuaWeimu|LuaJiejiaxuWeimu', enemies[1]) then return false end
	if getCardsNum('Jink', to, from) < 1 or sgs.card_lack[to:objectName()]['Jink'] == 1 or self:isWeak(to) then return false end
	if to:getHandcardNum() >= 3 and to:hasSkill('LuaZaoxian') then return true end
	return false
end
...
LuaJixi_skill.getTurnUseCard = function(self)
	...
	for i = 0, self.player:getPile('field'):length() - 1, 1 do
		...
		local snatch_str = ('snatch:LuaJixi[%s:%s]=%d'):format(snatch:getSuitString(), snatch:getNumberString(), self.player:getPile('field'):at(i))
		local LuaJixisnatch = sgs.Card_Parse(snatch_str)

		for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if (self.player:distanceTo(player, 1) <= 1 + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_DistanceLimit, self.player, LuaJixisnatch))
				and not self.room:isProhibited(self.player, player, LuaJixisnatch) and self:hasTrickEffective(LuaJixisnatch, player) then
