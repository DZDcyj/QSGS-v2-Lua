...
Maxcards = sgs.CreateMaxCardsSkill{
	...
	extra_func = function(self, target)
		...
		if target:getMark("@hulaoguan") > 0 and target:hasSkill("shenwei") then n = n + 1 end
		return target:getMark("@Maxcards") + target:getMark("@Maxcards-Clear") - target:getMark("@zhongjian") - target:getMark("@jishe-Clear") + target:getMark("@ol_mingjian_flag") + n
	end
}
...
slashmore = sgs.CreateTargetModSkill{
	...
	residue_func = function(self, from, card, to)
		...
		if card:isKindOf("Slash") then
			...
			if from:hasSkill("jieyingy") and from:getMark("@thiefed") > 0 then n = n + 1 end
			if from:getMark("@hulaoguan") > 0 and from:hasSkill("shenji") and not from:getWeapon() then n = n + 1 end
			if from:hasSkill("huihuo") then n = n + 1 end
	...
	distance_limit_func = function(self, from, card, to)
		...
		if card:getSkillName() == "ol_shensu" or card:getSkillName() == "longnu_red" or card:getSkillName() == "yingjian" or card:getSkillName() == "shanjia" or from:getMark("kuangcai_replay") > 0 or from:getMark("sheyan") > 0 then
			n = n + 1000
		end
		for _, p in sgs.qlist(from:getAliveSiblings()) do
		...
		if sgs.GetConfig("starfire", true) and from:hasSkill("wusheng") and card and card:getSuit() == sgs.Card_Diamond and card:isKindOf("Slash") then
			n = n + 1000
		end
		if from:getMark("chenglve" .. card:getSuitString() .. "-Clear") > 0 then
			n = n + 1000
		end
		if card:isKindOf("SupplyShortage") and from:hasSkill("ol_duanliang") and to and to:getHandcardNum() >= from:getHandcardNum() then
			n = n + 1000
		end
		if from:hasSkill("limu") and not from:getJudgingArea():isEmpty() and to and from:inMyAttackRange(to) then
			n = n + 1000
...
distance = sgs.CreateDistanceSkill{
	...
	correct_func = function(self, from, to)
		...
		--[[for _, mark in sgs.list(to:getMarkNames()) do
			if string.find(mark, "wenji") and to:getMark(mark) > 0 and from:objectName() ~= string.sub(mark, 6, string.len(mark) - 6) then
				n = n + 1
			end
		end]]--
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.PreCardUsed then
			...
			if use.card then
				...
				if skill == "dingpan" or skill == "_mizhao" or skill == "duliang" or skill == "_ol_zhongyong" or skill == "jiyu" or skill == "kuangbi"  or skill == "fenyue" or skill == "_fenyue" or skill == "shuimeng" or skill == "_shensu" or skill == "shanjia" or skill == "_shanjia" or skill == "jixu" or skill == "qinguo" or skill == "poxi" then return true end
			end
		else
...
banbanban = sgs.CreateProhibitSkill{
	...
	is_prohibited = function(self, from, to, card)
		return not card:isKindOf("SkillCard") and ((card:getSkillName() == "jiaozhao" and from:objectName() == to:objectName()) or (from:hasSkill("caishi") and to:getMark("caishi-Clear") > 0) or (from:getMark("juzhanFrom-Clear") > 0 and to:getMark("juzhanTo-Clear") > 0 and not card:isKindOf("SkillCard")) or ((not to:hasEquipArea(0) and card:isKindOf("Weapon")) or (not to:hasEquipArea(1) and card:isKindOf("Armor")) or (not to:hasEquipArea(2) and card:isKindOf("DefensiveHorse")) or (not to:hasEquipArea(3) and card:isKindOf("OffensiveHorse")) or (not to:hasEquipArea(4) and card:isKindOf("Treasure")) or (not to:hasEquipArea() and card:isKindOf("EquipCard")) or ((to:hasSkill("yinship") or to:hasSkill("qianjie") or to:hasSkill("zhenlve") or not to:hasJudgeArea())and card:isKindOf("DelayedTrick")))) or card:hasFlag("lianji"..to:objectName())
	end
}
		...
		if event == sgs.DamageCaused then
			...
			if card and not card:isKindOf("SkillCard") and player:getPhase() == sgs.Player_Play then
				...
				if player:getMark("kuangcai_replay") > 0 then
					...
					if player:getMark("@kuangcaidraw_Play") >= 5 or (sgs.GetConfig("kuangcai_change", true) and player:getMark("@kuangcaidraw_Play") >= 2) then
...
wenguaCard = sgs.CreateSkillCard{
	...
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:hasSkill("wengua") and to_select:getMark("wengua_Play") == 0 and (sgs.Self:hasSkill("wengua") and to_select:getMark("bf_huashenxushi") == 0)
	end,
...
extra_targetCard = sgs.CreateSkillCard{
	...
	filter = function(self, targets, to_select)
		local x = 0
		if sgs.Self:getMark("fumian2_manmanlai") > 0 and sgs.Self:getMark("fumian2now_manmanlai") == 0 and sgs.Self:getPhase() ~= sgs.Player_NotActive then
			x = x + 1
...
extra_target = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if use.from:objectName() == player:objectName() and ((player:getMark("fumian2_manmanlai") > 0 and player:getMark("fumian2now_manmanlai") == 0) or (player:getMark("ol_fumian2_manmanlai") == 3 and player:getMark("stop_fumian_bug-Clear") == 0)) and use.card:isRed() and not use.card:isKindOf("Collateral") and not use.card:isKindOf("EquipCard") and not use.card:isKindOf("SkillCard") then
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if use.to:contains(p) or room:isProhibited(player, p, use.card) or not use.card:targetFilter(sgs.PlayerList(), p, player) then
...
poison_effect = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		local n = 0
		for _, id in sgs.qlist(move.card_ids) do
			if move.open and sgs.Sanguosha:getCard(id):isKindOf("Slash") then
				n = n + 1
			end
		end
...
wenji_buff = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.TrickCardCanceling then
			local effect = data:toCardEffect()
			if effect.from and effect.from:hasSkill(self:objectName()) and effect.from:getMark("wenji"..effect.card:getClassName().."-Clear") > 0 then return true end
		end
	end
}
...
people_fuck = sgs.CreateTriggerSkill{
	...
	global = true,
	on_trigger = function(self, event, player, data, room)
		local num,n,m,x,y,z = 0,0,0,0,0,0
		for _,p in sgs.qlist(room:getAlivePlayers()) do
...
if not sgs.Sanguosha:getSkill("distance") then skills:append(distance) end
if not sgs.Sanguosha:getSkill("slashmore") then skills:append(slashmore) end
...
if not sgs.Sanguosha:getSkill("Maxcards") then skills:append(Maxcards) end
...
if not sgs.Sanguosha:getSkill("#banbanban") then skills:append(banbanban) end
...
if not sgs.Sanguosha:getSkill("Fake_Move") then skills:append(Fake_Move) end
...
if not sgs.Sanguosha:getSkill("skill_mark") then skills:append(skill_mark) end
...
if not sgs.Sanguosha:getSkill("people_fuck") then skills:append(people_fuck) end
if not sgs.Sanguosha:getSkill("axe_bug") then skills:append(axe_bug) end
...
function ChangeCheck(player, name)
	if player:getGeneralName() == name or player:getGeneral2Name() == name then
		...
		player:getRoom():changeHero(player, name, false, false, player:getGeneral2Name() and player:getGeneral2Name() == name, false)
...
hanyong = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardUsed and player:getHp() < room:getLord():getMark("@clock_time") then
			local use = data:toCardUse()
			if (use.card:isKindOf("SavageAssault") or use.card:isKindOf("ArcheryAttack")) and room:askForSkillInvoke(player, self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName())
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
...
shanjia = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play and x > 0 and player:hasSkill(self:objectName()) and room:askForSkillInvoke(player, self:objectName(), data) then
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
				player:drawCards(x)
	...
	on_effect = function(self, effect)
		...
		if effect.from:getMark(self:objectName().."engine") > 0 then
			...
			if not use_slash and not effect.to:getEquips():isEmpty() then
				local card = room:askForCard(effect.to, ".|.|.|equipped!", "@zhige_give", sgs.QVariant(), sgs.Card_MethodNone)
				if card then
					room:moveCardTo(card, effect.from, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, effect.to:objectName(), effect.from:objectName(), self:objectName(), ""))
				end
			end
...
duliang = sgs.CreateDrawCardsSkill{
	...
	draw_num_func = function(self, player, n)
		...
		if player:getMark("@hulaoguan") > 0 and player:hasSkill("shenwei") > 0 then x = x + 1 end
		if x > 0 then
...
--[[fulin = sgs.CreateMaxCardsSkill{--擦边球写法，手牌上限+X（X为你手牌中此回合获得的牌数）
	name = "fulin", 
	extra_func = function(self, target)
		local x = 0
		for _, card in sgs.list(target:getHandcards()) do
			if target:getMark(self:objectName()..card:getId().."-Clear") > 0 and target:hasSkill(self:objectName()) and target:getPhase() == sgs.Player_Discard then
				x = x + 1
			end
			if target:getMark("luoshen"..card:getId().."-Clear") > 0 and target:getPhase() == sgs.Player_Discard then
				x = x + 1
			end
...
huanghao = sgs.General(extension6, "huanghao", "shu", 3, true, sgs.GetConfig("hidden_ai", true) and sgs.GetConfig("huanghao_down", true))
...
qinqing = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		...
		local players = sgs.SPlayerList()
		for _, p in sgs.qlist(room:getAlivePlayers()) do
			if room:getLord() and p:inMyAttackRange(room:getLord()) and (p:objectName() ~= player:objectName() or sgs.GetConfig("huanghao_down", true)) and (player:canDiscard(p, "he") and not p:isNude()) or (sgs.GetConfig("huanghao_down", true) and p:isNude()) then
				players:append(p)
			end
		end
...
huishengVS = sgs.CreateViewAsSkill{
	...
	n = 999,
	view_filter = function(self, selected, to_select)
		if #selected > 0 and #selected < sgs.Self:getMark("huisheng") - 1 and not selected[1]:hasFlag("huisheng") then return not to_select:hasFlag("huisheng") end
...
guizao = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if player:getPhase() == sgs.Player_Discard and move.from and move.from:objectName() == player:objectName() and move.from:hasSkill(self:objectName()) and (bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_DISCARD) then
				local guizao_ex = {}
				for _, id in sgs.qlist(move.card_ids) do
		...
		elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Discard and player:getMark("guizao_biu") >= 2 and RIGHT(self, player) then
...
jili = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		local use = data:toCardUse()
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if use.card and use.card:isRed() and use.from:objectName() ~= p:objectName() and not use.to:contains(p) and (use.card:isKindOf("BasicCard") or use.card:isNDTrick()) and not use.card:isKindOf("Collateral") and player:distanceTo(p) == 1 and not room:isProhibited(use.from, p, use.card) then
				local n = 1
...
luanzhan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.PreCardUsed then
			local use = data:toCardUse()
			if use.from:objectName() == player:objectName() and player:getMark("@luanz") > 0 and (use.card:isKindOf("Slash") or (use.card:isNDTrick() and use.card:isBlack() and not use.card:isKindOf("Collateral") and not use.card:isKindOf("Nullification"))) then
				for _, p in sgs.qlist(use.to) do
					room:addPlayerMark(p, self:objectName())
				end
				room:setPlayerMark(player, "card_id", use.card:getEffectiveId())
		...
		elseif event == sgs.TargetSpecified then
			local use = data:toCardUse()
			if use.from:objectName() == player:objectName() and (use.card:isKindOf("Slash") or (use.card:isNDTrick() and use.card:isBlack())) and use.to:length() < player:getMark("@luanz") then
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
	...
	on_use = function(self, room, source, targets)
		...
		if source:getMark(self:objectName().."engine") > 0 then
			...
			for _, p in sgs.qlist(room:getOtherPlayers(targets[1])) do
				if p:inMyAttackRange(targets[1]) and p:objectName() ~= source:objectName() and targets[1]:canPindian(p, self:objectName()) then
					players:append(p)
				end
			end
		...
		for _,p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if player:getPhase() == sgs.Player_Play and p:objectName() ~= player:objectName() and p:hasEquip() then
				if event == sgs.EventPhaseStart then
					...
					if player:hasFlag(self:objectName()) and player:getMark("used_slash-Clear") == 0 and p:canSlash(player, nil, false) then
						local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
...
ol_liegong = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		local use = data:toCardUse()
		local room = player:getRoom()
		if event == sgs.TargetSpecified and player:objectName() == use.from:objectName() and use.from:hasSkill(self:objectName()) and use.card:isKindOf("Slash") then
			local index, up = 1, 0
			local jink_table = sgs.QList2Table(player:getTag("Jink_" .. use.card:toString()):toIntList())
			for _, p in sgs.qlist(use.to) do
...
fuhan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if player:getPhase() == sgs.Player_RoundStart and x > 0 and player:hasSkill("fanghun") and player:getMark("@fuhan") > 0 and room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("up:"..x)) then
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
...
kuanshi = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		local room = player:getRoom()
		if player:getPhase() == sgs.Player_Finish then
			local target = room:askForPlayerChosen(player, room:getAlivePlayers(), self:objectName(), "kuanshi-invoke", true, sgs.GetConfig("face_game", true))
			if target then
				room:broadcastSkillInvoke(self:objectName(), 1)		
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
...
jiwu = sgs.CreateOneCardViewAsSkill{
	...
	enabled_at_play = function(self, player)
		return not player:hasSkill("qiangxi") or not player:hasSkill("lieren") or not player:hasSkill("xuanfeng") or not player:hasSkill("wansha")
	end
}
					...
					if player:getMark(self:objectName().."engine") > 0 then
						...
						if to:getHandcardNum() == to:getHp() then
							...
							if to:objectName() ~= player:objectName() then
								room:askForYiji(player,getIntList(player:getCards("he")), self:objectName(), false, false, true, 1, players, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), to:objectName(), self:objectName(), ""), "bingzheng-distribute", true)
							end
						end
						room:removePlayerMark(player, self:objectName().."engine")
					end
...
yixiang = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if RIGHT(self, player) and use.from:getHp() > player:getHp() and not use.card:isKindOf("SkillCard") and not room:getCurrent():hasFlag(self:objectName()..player:objectName()) and use.to:contains(player) and room:askForSkillInvoke(player, self:objectName(), data) then
			room:getCurrent():setFlags(self:objectName()..player:objectName())
			room:broadcastSkillInvoke(self:objectName())
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
...
ol_wuji = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		local room = player:getRoom() 
		if player:getPhase() == sgs.Player_Finish and player:getMark(self:objectName()) == 0 and player:getMark("damage_point_round") >= 3 then
			room:broadcastSkillInvoke(self:objectName())
			...
			if to then
				...
				if player:getMark(self:objectName().."engine") > 0 then
					...
					for i = 0, x - 1 do
						...
						card_ids:append(room:askForCardChosen(player, to, "he", self:objectName(), false, sgs.Card_MethodDiscard))
...
xingzhao = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if event == sgs.CardsMoveOneTime then
				local move = data:toMoveOneTime()
				if ((move.from and move.from:objectName() == p:objectName() and player:objectName() == p:objectName() and move.from_places:contains(sgs.Player_PlaceEquip)) or (move.to and move.to:objectName() == p:objectName() and player:objectName() == p:objectName() and move.to_place == sgs.Player_PlaceEquip)) and ((p:getEquips():length() > 1 and not p:hasSkill("xunxun")) or (p:hasSkill("xunxun") and p:getEquips():length() <= 1)) then
					room:sendCompulsoryTriggerLog(p, self:objectName())
			...
			elseif event == sgs.CardUsed then
				local use = data:toCardUse()
				if use.card:isKindOf("EquipCard") and p:getEquips():length() > 2 and room:askForSkillInvoke(p, self:objectName(), data) then
					room:broadcastSkillInvoke(self:objectName(), 2)
					room:addPlayerMark(use.from, self:objectName().."engine")
					if use.from:getMark(self:objectName().."engine") > 0 then
						use.from:drawCards(1, self:objectName())
			...
			elseif event == sgs.EventPhaseChanging then
				if data:toPhaseChange().to == sgs.Player_Judge and p:getEquips():length() > 3 and room:askForSkillInvoke(p, self:objectName(), data) then
					room:broadcastSkillInvoke(self:objectName(), 1)
					room:addPlayerMark(p, self:objectName().."engine")
					if p:getMark(self:objectName().."engine") > 0 then
						player:skip(sgs.Player_Judge)
...
bug_caoren = sgs.General(extension_sp, "bug_caoren", "wei", 4, true, sgs.GetConfig("hidden_ai", true) or sgs.GetConfig("EnableHidden", true))
...
zhongjianCard = sgs.CreateSkillCard{
	...
	will_throw = false, 
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:getHandcardNum() > to_select:getHp() and to_select:objectName() ~= sgs.Self:objectName()
	end, 
	on_use = function(self, room, source, targets)
		room:addPlayerMark(source, self:objectName().."engine")
		if source:getMark(self:objectName().."engine") > 0 then
			...
			for i = 1, targets[1]:getHandcardNum() - targets[1]:getHp() do
	...
	on_phasechange = function(self, player)
		...
		if player:getPhase() == sgs.Player_Finish and not targets:isEmpty() and room:askForSkillInvoke(player, self:objectName()) then
			...
			if player:getMark(self:objectName().."engine") > 0 then
				...
				local cards = room:askForExchange(player, self:objectName(), math.min(player:getHandcardNum(), targets:length()), math.min(player:getHandcardNum(), targets:length()), false, "@xiashu-invoke", false)
				if cards then
					local ids = sgs.IntList()
					for _,id in sgs.qlist(cards:getSubcards()) do
			...
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				if player:getMark(self:objectName().."engine") > 0 then
					...
					if judge.card:isRed() then
						local to = room:askForPlayerChosen(player, room:getAlivePlayers(), self:objectName(), "chouce-invoke", true, true)
						if to then
							local x = 1
					...
					elseif judge.card:isBlack() then
						...
						if not targets:isEmpty() then
							local to = room:askForPlayerChosen(player, targets, self:objectName(), "chouce-invoke", true, true)
							if to then
								if to:getMark("fu"..player:objectName()) > 0 then
									room:broadcastSkillInvoke(self:objectName(), 2)
								end
...
daiyan = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		local room = player:getRoom()
		if player:getPhase() == sgs.Player_Start then
			local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@invoke:"..self:objectName(), true, true)
			if to then
				room:broadcastSkillInvoke(self:objectName(), 1)
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
		...
		if player:getPhase() == sgs.Player_Finish then
			...
			if to then
				...
				if player:getMark(self:objectName().."engine") > 0 then
					...
					local peachs = sgs.IntList()
					for _,id in sgs.qlist(room:getDrawPile()) do
						if sgs.Sanguosha:getCard(id):isKindOf("BasicCard") and sgs.Sanguosha:getCard(id):getSuit() == sgs.Card_Heart then
							peachs:append(id)
						end
					end
					if not peachs:isEmpty() then
...
ol_xiaoqiao = sgs.General(extension_god, "ol_xiaoqiao", "wu", 3, false, sgs.GetConfig("hidden_ai", true) or sgs.GetConfig("EnableHidden", true))
...
pizhuan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if (player:getPile("book"):length() < 4 and ((event == sgs.TargetConfirmed and use.to:contains(player) and use.from:objectName() ~= player:objectName()) or (event == sgs.CardUsed and use.from:objectName() == player:objectName()))) and use.card:getSuit() == sgs.Card_Spade and room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
	...
	on_trigger = function(self, event, player, data, room)
		...
		if use.card:isKindOf("Slash") then
			...
			if event == sgs.TargetSpecified then
				...
				if use.from and use.from:isAlive() and use.to:contains(player) and player:canPindian(use.from, self:objectName()) and room:askForSkillInvoke(player, self:objectName(), _data) then
					room:addPlayerMark(player, self:objectName().."engine")
					if player:getMark(self:objectName().."engine") > 0 then
...
tianbian = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.PindianVerifying and RIGHT(self, player) then
			local pindian = data:toPindian()
			if (pindian.from:objectName() == player:objectName() and pindian.from_card:getSuit() == sgs.Card_Heart) or (pindian.to:objectName() == player:objectName() and pindian.to_card:getSuit() == sgs.Card_Heart) then
...
jiexun = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.EventPhaseStart and RIGHT(self, player) and n > 0 and player:getPhase() == sgs.Player_Finish then
			local target = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "jiexun-invoke", true, true)
			if target then
				room:broadcastSkillInvoke(self:objectName(), 1)
				room:addPlayerMark(player, self:objectName().."engine")
					if player:getMark(self:objectName().."engine") > 0 then
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.StartJudge then
			if not player:getPile("incantation"):isEmpty() then 
				...
				room:addPlayerMark(player, self:objectName().."-Clear")
				for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					room:removePlayerMark(player, self:objectName()..player:getPile("incantation"):first()..p:objectName())
				end
			end
		else
...
zishu = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local move = data:toMoveOneTime()
		if event == sgs.CardsMoveOneTime and not room:getTag("FirstRound"):toBool() and move.to and move.to:objectName() == player:objectName() then
...
yingyuan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.CardsMoveOneTime then
			...
			if move.from_places:contains(sgs.Player_PlaceTable) and move.to_place == sgs.Player_DiscardPile and bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_USE and (move.from and move.from:objectName() == player:objectName() or invoke) and player:getPhase() ~= sgs.Player_NotActive then
				local dummy = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
				for _,id in sgs.qlist(move.card_ids) do
...
pindiCard = sgs.CreateSkillCard{
	name = "pindi" ,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:getMark(self:objectName().."_Play") == 0 and to_select:objectName() ~= sgs.Self:objectName()
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
		...
		if effect.from:getMark(self:objectName().."engine") > 0 then
			...
			room:addPlayerMark(effect.from, self:objectName()..sgs.Sanguosha:getCard(self:getSubcards():first()):getTypeId().."_Play")
...
zongkui = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if not players:isEmpty() then
			local target = room:askForPlayerChosen(player, players, self:objectName(), "zongkui-invoke", event == sgs.TurnStart, true)
			if target then
				if event == sgs.TurnStart then
					room:broadcastSkillInvoke(self:objectName(), 2)
				else
...
canshibEXCard = sgs.CreateSkillCard{
	...
	filter = function(self, targets, to_select)
		...
		local tos = sgs.Self:property("extra_collateral_current_targets"):toString():split("+")
		if #targets == 0 then
			return not table.contains(tos, to_select:objectName()) and not sgs.Self:isProhibited(to_select, coll) and coll:targetFilter(targetsTable2QList(targets), to_select, sgs.Self) and to_select:getMark("@puppet") > 0
		else
			return coll:targetFilter(targetsTable2QList(targets), to_select, sgs.Self)
		end
	end, 
...
canshib = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if use.to:length() == 1 then
			for _,p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if use.from:getMark("@puppet") > 0 and use.to:contains(p) and room:askForSkillInvoke(p, self:objectName(), data) then
					room:addPlayerMark(p, self:objectName().."engine")
					if p:getMark(self:objectName().."engine") > 0 then
						room:broadcastSkillInvoke(self:objectName())
				...
				elseif use.from:objectName() == p:objectName() and use.card:isKindOf("Collateral") then
					for i = 1, p:getMark("@luanz") do
						for _, pe in sgs.qlist(room:getAlivePlayers()) do
							if (use.to:contains(pe) or room:isProhibited(p, pe, use.card)) then continue end
						...
						if not targets:isEmpty() then
							...
							room:setPlayerProperty(p, "extra_collateral_current_targets", sgs.QVariant(table.concat(tos, "+")))
...
weijingVS = sgs.CreateZeroCardViewAsSkill{
	...
	enabled_at_response = function(self, player, pattern)
		return (pattern == "slash" or pattern == "jink") and player:getMark(self:objectName().."_lun") == 0 and sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE
	end
}
	...
	on_trigger = function(self, event, player, data, room)
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if event == sgs.Death then
				local death = data:toDeath()
				if death.who:objectName() == player:objectName() and death.damage.reason and death.damage.reason == self:objectName() then
					room:setTag(self:objectName(), sgs.QVariant(true))
				end
			elseif event == sgs.Damage then
				local damage = data:toDamage()
				if p and damage.to:objectName() == p:objectName() and p:getMark(self:objectName()) == 1 and damage.from and damage.from:objectName() ~= p:objectName() and room:askForSkillInvoke(p, self:objectName(), data) then
...
juzhan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		if event == sgs.TargetConfirmed then
			...
			if use.card:isKindOf("Slash") and player:getMark(self:objectName()) ~= 1 and use.from and use.to:contains(player) and use.from:objectName() ~= player:objectName() and room:askForSkillInvoke(player, self:objectName(), _data) then
			...
			for _, p in sgs.qlist(use.to) do
				_data:setValue(p)
				if use.card:isKindOf("Slash") and player:getMark(self:objectName()) == 1 and not p:isNude() and room:askForSkillInvoke(player, self:objectName(), _data) then
					ChangeSkill(self, room, player)
					room:broadcastSkillInvoke(self:objectName(), 1)
					room:addPlayerMark(player, self:objectName().."engine")
					if player:getMark(self:objectName().."engine") > 0 then
...
feijunCard = sgs.CreateSkillCard{
	...
	filter = function(self, targets, to_select)
		...
		return #targets == 0 and (to_select:getHandcardNum() > sgs.Self:getHandcardNum() - n or to_select:getEquips():length() > sgs.Self:getEquips():length() - m) and to_select:objectName() ~= sgs.Self:objectName()
	end, 
	on_use = function(self, room, source, targets)
		local choices = {}
...
yili = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		...
		if player:getPhase() == sgs.Player_Play then
			room:broadcastSkillInvoke(self:objectName())
			local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "yili-invoke", true, true)
			if to then
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
...
zhenglun = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local change = data:toPhaseChange()
		if change.to == sgs.Player_Draw and not player:isSkipped(sgs.Player_Draw) and player:getMark("@orange") == 0 and room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
				player:skip(sgs.Player_Draw)
...
chenglveCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		if source:getMark(self:objectName().."engine") > 0 then
			...
			local cards = room:askForExchange(source, self:objectName(), math.min(source:getHandcardNum(), x), x, false, prompt)
			if cards then
...
ol_shicai = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local card = data:toCardUse().card
		if player:getMark("shicai"..card:getTypeId().."-Clear") == 0 and not card:isKindOf("SkillCard") and RIGHT(self, player) and room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
...
zhenliangVS = sgs.CreateViewAsSkill{
	...
	enabled_at_play = function(self, player)
		return player:canDiscard(player, "he") and not player:hasUsed("#zhenliang") and player:getMark(self:objectName()) ~= 1 and player:getPile("ren"):length() > 0
	end
}
zhenliang = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeCardsMove then
			...
			if player:getPhase() == sgs.Player_NotActive and player:getMark(self:objectName()) == 1 and not player:getPile("ren"):isEmpty() and invoke then
...
bukuishishen = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.Damage and (player:hasSkill("duorui") or player:hasSkill("zhiti")) then
			...
			if player:hasSkill("duorui") and damage.to:objectName() ~= player:objectName() and player:getPhase() == sgs.Player_Play and player:getMark("duorui_lun") == 0 and player:hasEquipArea() and #duoruis > 0 then
			...
			if player:hasSkill("zhiti") and damage.card and damage.card:isKindOf("Duel") and invoke and damage.to:isWounded() and player:inMyAttackRange(damage.to) then
			...
			if #choices > 0 then
				...
				if player:hasSkill("duorui") and damage.to:objectName() ~= player:objectName() and player:getPhase() == sgs.Player_Play and player:getMark("duorui_lun") == 0 and player:hasEquipArea() and #duoruiss > 0 then
				...
				if player:hasSkill("zhiti") and damage.card:isKindOf("Duel") and invoke and damage.to:isWounded() and player:inMyAttackRange(damage.to) then
		...
		elseif event == sgs.DamageInflicted then
			for _, p in sgs.qlist(room:getAlivePlayers()) do
				...
				if damage.nature ~= sgs.DamageStruct_Normal and not damage.chain and damage.to:isChained() and n > 0 and p:hasSkill("chenghao") then
					table.insert(choices, "chenghao")
				end
				if p:getMark("@dragon") + p:getMark("@phoenix") == 0 and not p:getArmor() and (damage.nature ~= sgs.DamageStruct_Normal or (damage.card and damage.card:isKindOf("TrickCard"))) and p:hasSkill("yinshi") then
...
wanglie = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.TrickCardCanceling then
			...
			if card and not card:isKindOf("SkillCard") and player:getPhase() == sgs.Player_Play and room:askForSkillInvoke(player, self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName())
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
...
hongju = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		local room = player:getRoom() 
		if player:getPhase() == sgs.Player_Start and player:getMark(self:objectName()) == 0 and player:getPile("honor"):length() >= 3 then
			local invoke = false
			for _, p in sgs.qlist(room:getAllPlayers(true)) do
				if p:isDead() then
					invoke = true
...
kongshengVS = sgs.CreateViewAsSkill{
	...
	view_filter = function(self, selected, to_select)
		if string.endsWith(sgs.Sanguosha:getCurrentCardUsePattern(), "!") then
			return sgs.Self:getPile("music"):contains(to_select:getEffectiveId()) and to_select:isKindOf("EquipCard") and not sgs.Self:isProhibited(sgs.Self, to_select) and to_select:isAvailable(sgs.Self)
		else
	...
	on_phasechange = function(self, player)
		...
		elseif player:getPhase() == sgs.Player_Finish then
			...
			for _,id in sgs.qlist(player:getPile("music")) do
				if sgs.Sanguosha:getCard(id):isKindOf("EquipCard") and not player:isProhibited(player, sgs.Sanguosha:getCard(id)) and sgs.Sanguosha:getCard(id):isAvailable(player) then
					invoke = true
				end
			end
...
poshi = sgs.CreatePhaseChangeSkill{
	...
	on_phasechange = function(self, player)
		local room = player:getRoom()
		if player:getPhase() == sgs.Player_Start and (not player:hasEquipArea() or player:getHp() == 1) and player:getMark(self:objectName()) == 0 then
			SendComLog(self, player)
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
				room:addPlayerMark(player, self:objectName())
...
jieyingy = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseChanging and data:toPhaseChange().to == sgs.Player_NotActive then
			...
			elseif event == sgs.EventPhaseStart and RIGHT(self, player) and not players:isEmpty() and player:getMark("@thiefed") > 0 and player:getPhase() == sgs.Player_Finish then
...
xunxian = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.BeforeCardsMove then
			...
			if player:getPhase() == sgs.Player_NotActive and move.from and (move.from:objectName() == player:objectName() or invoke) and move.to_place == sgs.Player_DiscardPile and (extract == sgs.CardMoveReason_S_REASON_USE or extract == sgs.CardMoveReason_S_REASON_RESPONSE) then
				if not room:getCurrent():hasFlag(self:objectName()..player:objectName()) then
					local targets = sgs.SPlayerList()
					for _, p in sgs.qlist(room:getOtherPlayers(player)) do 
						if p:getHandcardNum() > player:getHandcardNum() then
...
yingshi = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.Damage then
			local damage = data:toDamage()
			if damage.from and damage.to and not damage.to:getPile("reward"):isEmpty() and damage.card and damage.card:isKindOf("Slash") then
				...
				if id ~= -1 then
					room:obtainCard(damage.from, sgs.Sanguosha:getCard(id), sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_EXCHANGE_FROM_PILE, damage.from:objectName()))
				end
				room:clearAG(damage.from)
			end
...
--[[sanwenCard = sgs.CreateSkillCard{
	name = "sanwen",
    target_fixed = true,
	will_throw = false,
	on_use = function(self, room, source, targets)
		if source:isAlive() then
			room:setPlayerFlag(room:getCurrent(), self:objectName()..source:objectName())
			local dummy = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
			for _, card in sgs.qlist(source:getHandcards()) do
				for _,id in sgs.qlist(self:getSubcards())do
					if card:hasFlag(self:objectName()) and TrueName(sgs.Sanguosha:getCard(id)) then
						dummy:addSubcard(card:getId())
					end
				end
			end
			if dummy:subcardsLength() > 0 then
				...
				source:drawCards(dummy:subcardsLength()*2, self:objectName())
			end
		end
	end
}
sanwenVS = sgs.CreateViewAsSkill{
	name = "sanwen",
	n = 999,
	view_filter = function(self, selected, to_select)
        return not sgs.Self:isJilei(to_select) and sgs.Self:getMark(self:objectName()..TrueName(to_select)) > 0 and not to_select:hasFlag(self:objectName())
	end,
	view_as = function(self, cards)
		if #cards == 0 then return nil end
		local zhiheng_card = sanwenCard:clone()
	...
	enabled_at_response = function(self, player, pattern)
		return pattern == "@sanwen" and player:canDiscard(player, "h")
	end
}
sanwen = sgs.CreateTriggerSkill{
	name = "sanwen",
	view_as_skill = sanwenVS, 
	events = {sgs.CardsMoveOneTime},
	on_trigger = function(self, event, player, data, room)
		local move = data:toMoveOneTime()
		if not room:getCurrent():hasFlag(self:objectName()..player:objectName()) and not room:getTag("FirstRound"):toBool() and move.to and move.to:objectName() == player:objectName() and move.to_place == sgs.Player_PlaceHand then
			...
			for _,id in sgs.qlist(move.card_ids) do
				if room:getCardOwner(id):objectName() == player:objectName() and room:getCardPlace(id) == sgs.Player_PlaceHand then
					ids:append(id)
					room:setCardFlag(sgs.Sanguosha:getCard(id), self:objectName())
					room:addPlayerMark(player, self:objectName()..TrueName(sgs.Sanguosha:getCard(id)))
				end
			end
			if not ids:isEmpty() and player:getHandcardNum() > move.card_ids:length() and player:canDiscard(player, "h") then
				room:askForUseCard(player, "@sanwen", "@sanwen", -1, sgs.Card_MethodNone)
			end
			for _, mark in sgs.list(player:getMarkNames()) do
...
guanwei = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			if player:getPhase() == sgs.Player_Play and not player:hasFlag(self:objectName()..p:objectName()) and player:getMark("used_Play") > 1 and player:getMark("guanwei_break-Clear") == 0 and room:askForCard(p, "..", "@guanwei", data, self:objectName()) then
				room:broadcastSkillInvoke(self:objectName())	
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
...
songsang = sgs.CreateTriggerSkill{
	...
	limit_mark = "@songsang",
	on_trigger = function(self, event, player, data, room)
		if player:getMark("@songsang") > 0 and data:toDeath().who:objectName() ~= player:objectName() and room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
...
zhanji = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		local move = data:toMoveOneTime()
		if not room:getTag("FirstRound"):toBool() and move.to and move.to:objectName() == player:objectName() and move.reason.m_reason == sgs.CardMoveReason_S_REASON_DRAW and move.to_place == sgs.Player_PlaceHand and move.reason.m_skillName ~= self:objectName() and player:getPhase() == sgs.Player_Play then
			SendComLog(self, player)
			room:addPlayerMark(player, self:objectName().."engine")
			if player:getMark(self:objectName().."engine") > 0 then
				player:drawCards(1, self:objectName())
...
qinguo = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if ((move.to and move.to:objectName() == player:objectName() and move.to_place == sgs.Player_PlaceEquip) or (move.from and move.from:objectName() == player:objectName() and move.from_places:contains(sgs.Player_PlaceEquip))) and player:getHp() == player:getEquips():length() and player:getEquips():length() ~= player:getMark(self:objectName()) and player:isWounded() and room:askForSkillInvoke(player, self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName(), 2)
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
					room:recover(player, sgs.RecoverStruct(player))
...
kannanCard = sgs.CreateSkillCard{
	...
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and sgs.Self:canPindian(to_select, self:objectName()) and to_select:getMark(self:objectName().."_Play") == 0
	end,
	on_use = function(self, room, source, targets)
...
kannanVS = sgs.CreateZeroCardViewAsSkill{
    ...
    enabled_at_play = function(self, player)
        return player:usedTimes("#kannan") < player:getHp() and player:getMark(self:objectName().."source_Play") == 0 and player:canPindian()
    end
}
kannan = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.ConfirmDamage then
			...
			if damage.from and damage.to:getMark("wusheng-Clear"..damage.from:objectName()) > 0 and damage.card and damage.card:isKindOf("Slash") and damage.card:getSuit() == sgs.Card_Heart then
				local log = sgs.LogMessage()
				log.type = "$kannan"
				log.from = player
				log.card_str = damage.card:toString()
...
zengdaoVS = sgs.CreateViewAsSkill{
    ...
    expand_pile = "sword",
	view_filter = function(self, selected, to_select)
		return (not string.endsWith(sgs.Sanguosha:getCurrentCardUsePattern(), "!") and to_select:isEquipped()) or (string.endsWith(sgs.Sanguosha:getCurrentCardUsePattern(), "!") and sgs.Self:getPile("sword"):contains(to_select:getEffectiveId()))
	end, 
	view_as = function(self, cards)
		if #cards > 0 then
...
zengdao = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseStart and RIGHT(self, player) and player:getPhase() == sgs.Player_Finish and player:getMark("@donate") > 0 and not player:getEquips():isEmpty() then
		...
		elseif event == sgs.DamageCaused and not player:getPile("sword"):isEmpty() then
			if not room:askForUseCard(player, "@zengdao!", "@zengdao", -1, sgs.Card_MethodNone) then
				room:throwCard(sgs.Sanguosha:getCard(player:getPile("sword"):first()), sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_REMOVE_FROM_PILE, "", player:objectName(), self:objectName(), ""), nil)
			end
			local damage = data:toDamage()
			damage.damage = damage.damage + 1
			data:setValue(damage)
...
dianhu = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.GameStart and RIGHT(self, player) then
			local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "dianhu-invoke", false, sgs.GetConfig("face_game", true))
			if to then
				room:broadcastSkillInvoke(self:objectName(), 1)
				room:addPlayerMark(player, self:objectName().."engine")
				if player:getMark(self:objectName().."engine") > 0 then
			...
			local damage = data:toDamage()
			for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				if player:getMark("aim"..p:objectName()) > 0 and player:isAlive() and damage.from:getKingdom() == p:getKingdom() and room:askForSkillInvoke(p, self:objectName(), data) then
					SendComLog(self, p, 2)
...
jianjiCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		local moves = sgs.CardsMoveList()
		local move = sgs.CardsMoveStruct(card_ids, nil, targets[1], sgs.Player_DrawPile, sgs.Player_PlaceHand, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DRAW, targets[1]:objectName(), "jianji", ""))
		moves:append(move)
...
sgs.LoadTranslationTable{
    ...
    ["$hongde2"] = "江南重义，东吴尚德。",
    ["dingpan"] = "定叛",
    [":dingpan"] = "<font color=\"green\"><b>出牌阶段限X次，</b></font>你可以选择装备区里有牌的一名角色，令其摸一张牌，然后其选择一项：1.令你弃置其装备区里的一张牌；2.其获得其装备区里的所有牌，你对其造成1点伤害。（X为反贼数）",
    ["$dingpan1"] = "从孙者生，从刘者死！",
    ["$dingpan2"] = "多行不义，必自毙！",
    ["~buzhi"] = "交州已定，主公~尽可放心......",
    ...
    [":fuhan"] = "限定技，回合开始时，若X大于0且你拥有“芳魂”，你可以先弃所有“梅影”标记再观看五张未登场的蜀势力武将牌，将此武将牌替换为其中一张，然后令体力上限为X，若你为体力值最少的角色，你回复1点体力。（X为你弃置过的“梅影”标记数且至多为游戏开始时的角色数）",
    ["$fuhan1"] = "承先父之志，扶汉兴刘~",
    ["$fuhan2"] = "天将降大任于我！",
    ["~zhaoxiang"] = "遁入阴影之中...",
   ...
   -- [":xingzhao"] = "锁定技，若你拥有的装备牌数大于：1，与你势力相同的角色可以令你选择是否令其发动“恂恂”；2.当与你势力相同的角色使用装备牌时，你选择是否令其摸一张牌；3.你选择是否跳过与你势力相同的角色的判定阶段。",
   -- [":xingzhao"] = "锁定技，若你拥有的装备牌数大于：1，与你势力相同的角色可以令你选择是否令其发动“恂恂”；2.当与你势力相同的角色使用装备牌时，你选择是否令其摸一张牌；3.你选择是否跳过与你势力相同的角色的弃牌阶段；4且所有与你势力相同的角色与你的胜利条件相同，与你胜利条件相同的角色胜利。",
    ["$xingzhao1"] = "拿些上好的木料来。",
    ...
    ["zhongjian"] = "忠鉴",
    [":zhongjian"] = "出牌阶段限一次，你可以展示一张手牌并展示手牌数大于体力值的一名其他角色X张手牌，若其以此法展示的牌与你展示的牌之中：有颜色相同的牌，你选择是否弃置其一张牌，若选择否，你摸一张牌；有点数相同的牌，此技能于此回合内视为“出牌阶段限两次”；均没有，你的手牌上限-1。（X为其手牌数和体力值之差）",
    [":zhongjian_buff"] = "出牌阶段限两次，你可以展示一张手牌并展示手牌数大于体力值的一名其他角色X张手牌，若其以此法展示的牌与你展示的牌：有颜色相同的牌，你选择是否弃置其一张牌，若选择否，你摸一张牌；没有颜色相同的牌且没有点数相同的牌，你的手牌上限-1。（X为其手牌数和体力值之差）",
    ["zhongjian1"] = "弃置其一张牌",
    ...
    ["wengua1"] = "置于牌堆顶",
    ["wengua2"] = "置于牌堆底",
    [":wengua"] = "<font color=\"green\"><b>每名角色的出牌阶段限一次，</b></font>若其：不为你，其可以交给你一张手牌，你可以将之置于牌堆顶/牌堆底，若如此做，你与其各从牌堆底/牌堆顶获得一张牌；为你，你可以将一张手牌置于牌堆顶/牌堆底，若如此做，你从牌堆底/牌堆顶获得一张牌。",
    ["$wengua1"] = "卦不能佳，可须异日。",
    ...
    ["lianji"] = "连计",
    [":lianji"] = "<font color=\"green\"><b>出牌阶段限一次，</b></font>你可以将一张【杀】或黑色锦囊牌交给一名角色，令其使用牌堆里随机一张武器牌。然后其选择一项：1.使用你以此法交给其的牌且你不是此牌的合法目标，然后将装备区里的武器牌交给其中一个目标；2.你视为对其使用你以此法交给其的牌，然后将装备区内的武器牌交给你。",
    ["$lianji1"] = "",
    ["$lianji2"] = "",
    ["moucheng"] = "谋逞",
    ...
    ["$suizhong2"] = "",
    ["cuiku"] = "摧枯",
    --[":cuiku"] = "游戏开始时，或当轮数为6的倍数时，你可以选择一至X名其他角色，对这些角色各造成2点伤害。（若你为：普通年兽，X为2；困难年兽，X为3。否则X为1）",
    [":cuiku"] = "游戏开始时，或当轮数为6的倍数时，若你：为简单年兽，你可以选择一名其他角色，对其造成2点伤害；为普通年兽，你可以选择一至两名其他角色，对这些角色各造成2点伤害；为困难年兽，对所有其他角色各造成为其体力值一半的伤害，然后你摸等同于其中体力上限为奇数的角色数的牌。",
    [":cuiku_sp"] = "当轮数为6的倍数时，若你：为简单年兽，你可以选择一名其他角色，对其造成2点伤害；为普通年兽，你可以选择一至两名其他角色，对这些角色各造成1点伤害；为困难年兽，你选择三名体力值大于1的其他角色，对这些角色各造成1点伤害，然后你摸一张牌。",
    ["$cuiku1"] = "",
    ["$cuiku2"] = "",
    ["~easy_nianshou"] = "",
    ...
    [":nianyi_sp"] = "锁定技，你使用牌无距离限制；锁定技，准备阶段开始时，若你的判定区里有牌，你随机弃置区域里的一张牌，然后若你的判定区里有牌，重复此流程；锁定技，其他角色的回合结束后，若你为困难年兽且你于此回合内失去过至少三张牌，你对当前回合角色造成1点伤害。",
    ["$nianyi1"] = "",
    ["$nianyi2"] = "",
    ["~normal_nianshou"] = "",
    ...
    ["@jianjie"] = "你可以发动“荐杰”",
    ["~jianjie"] = "选择角色→点击确定",
    [":jianjie"] = "第一个准备阶段开始时，你选择两名角色，其中一名角色获得1枚“龙印”标记，令一名角色获得1枚“凤印”标记；出牌阶段限一次（若此回合不是你的第一个回合，你选择一名拥有1“龙印”/“凤印”标记的角色）或当拥有“龙印”/“凤印”标记的角色死亡时，你可以令其将其拥有的“龙印”/“凤印”标记的角色弃所有“龙印”/“凤印”标记，然后令另一名角色获得1枚“龙印”/“凤印”标记；拥有“龙印”/“凤印”的角色拥有“火计”/“连环”，以此法获得的技能每名角色的回合限发动三次；拥有“龙印”和“凤印”的角色拥有“业炎”，以此法获得的技能发动后，其弃所有“龙印”标记和“凤印”标记。",
    ["$jianjie1"] = "",
    ["$jianjie2"] = "",
    ["chenghao"] = "称好",
    ...
    ["#pangdegong"] = "德懿举世",
    ["pingcai"] = "评才",
    [":pingcai"] = "出牌阶段限一次，你可以选择一项：1.若所有角色中有诸葛亮（火），你对一至两名角色各造成1点火焰伤害，否则你对一名角色造成1点火焰伤害；2.若所有角色中有庞统，你横置一至四名角色，否则你横置一至三名角色；3.若所有角色中有司马徽，你将一名角色装备区里的装备牌置入另一名角色的装备区，否则你将一名角色装备区里的防具牌置入另一名角色的装备区；4.若所有角色中有徐庶，你令一名角色摸一张牌，其回复1点体力，然后你摸一张牌，否则你令一名角色摸一张牌，其回复1点体力。",
    ["pingcai_wolong"] = "卧龙",
    ...
    [":longnu1"] = "转换技，锁定技，出牌阶段开始时，①你失去1点体力并摸一张牌，令你于此回合内红色手牌视为火【杀】且无距离限制；<font color=\"#01A5AF\"><s>②你减1点体力上限并摸一张牌，令你于此阶段内锦囊牌视为雷【杀】且无次数限制</s></font>。",
    [":longnu2"] = "转换技，锁定技，出牌阶段开始时，<font color=\"#01A5AF\"><s>①你失去1点体力并摸一张牌，令你于此回合内红色手牌视为火【杀】且无距离限制</s></font>；②你减1点体力上限并摸一张牌，令你于此阶段内锦囊牌视为雷【杀】且无次数限制。",
    ...
    [":cuike"] = "出牌阶段开始时，若你拥有的“军略”标记数为：奇数，你可以对一名角色造成1点伤害；偶数，你可以横置一名角色，然后弃置其一张牌。然后若你拥有的“军略”标记数大于7，你可以弃所有“军略”标记，对所有其他角色各造成1点伤害。",
    ["$cuike1"] = "摧敌心神，克敌计谋。",
    ["$cuike2"] = "克险摧难，军略当先。",
    ["zhanhuo"] = "绽火",
    ["@zhanhuo"] = "你可以发动“绽火”",
    ...
    [":juzhan1"] = "转换技，①当你成为其他角色使用【杀】的目标后，你可以与其各摸一张牌，然后其本回合不能对你使用牌；<font color=\"#01A5AF\"><s>②当你使用【杀】指定一名角色为目标后，你可以获得其一张牌，然后你本回合不能对其使用牌</s></font>。",
    [":juzhan2"] = "转换技，<font color=\"#01A5AF\"><s>①当你成为其他角色使用【杀】的目标后，你可以与其各摸一张牌，然后其本回合不能对你使用牌</s></font>；②当你使用【杀】指定一名角色为目标后，你可以获得其一张牌，然后你本回合不能对其使用牌。",
    ["$juzhan1"] = "砍头便砍头，何为怒邪！",
    ...
    [":shenshi"] = "转换技，①出牌阶段限一次，你可以将一张牌交给一名手牌数最多的角色，对其造成1点伤害，然后若其以此法死亡，你可以选择一名角色，其将手牌补至四张；②当其他角色对你造成伤害后，你可以观看其的手牌，然后交给其一张牌，若如此做，当前回合结束时，若其未失去你以此法交给其的牌，你将手牌补至四张。",
    [":shenshi1"] = "转换技，①出牌阶段限一次，你可以将一张牌交给一名手牌数最多的角色，对其造成1点伤害，然后若其以此法死亡，你可以选择一名角色，其将手牌补至四张；<font color=\"#01A5AF\"><s>②当其他角色对你造成伤害后，你可以观看其的手牌，然后交给其一张牌，若如此做，当前回合结束时，若其未失去你以此法交给其的牌，你将手牌补至四张</s></font>。",
    [":shenshi2"] = "转换技，<font color=\"#01A5AF\"><s>①出牌阶段限一次，你可以将一张牌交给一名手牌数最多的角色，对其造成1点伤害，然后若其以此法死亡，你可以选择一名角色，其将手牌补至四张</s></font>；②当其他角色对你造成伤害后，你可以观看其的手牌，然后交给其一张牌，若如此做，当前回合结束时，若其未失去你以此法交给其的牌，你将手牌补至四张。",
    ["$shenshi1"] = "深中足智，见时审情。",
    ["$shenshi2"] = "数语之言，审时度势。",
    ...
    [":chenglve1"] = "转换技，<font color=\"#01A5AF\"><s>①出牌阶段限一次，你可以摸一张牌，然后弃置两张手牌</s></font>；②你可以摸两张牌，然后弃置一张手牌。若如此做，你于此回合内使用与以此法弃置的牌相同花色的牌无距离和次数限制。",
    [":chenglve2"] = "转换技，①出牌阶段限一次，你可以摸一张牌，然后弃置两张手牌；<font color=\"#01A5AF\"><s>②你可以摸两张牌，然后弃置一张手牌</s></font>。若如此做，你于此回合内使用与以此法弃置的牌相同花色的牌无距离和次数限制。",
    ["$chenglve1"] = "成略在胸，良计速出。",
    ...
    ["ren"] = "任",
    ["zhenliang"] = "贞良",
    [":zhenliang"] = "转换技，①出牌阶段限一次，你可以选择一名攻击范围内的其他角色并弃置X张与“任”颜色相同的牌，对其造成1点伤害；②当你于回合外使用或打出的牌置入弃牌堆时，若此牌与“任”类别相同，你可以令一名角色摸一张牌。（X为你与其体力值之差且至少为1）",
    [":zhenliang1"] = "转换技，<font color=\"#01A5AF\"><s>①出牌阶段限一次，你可以选择一名攻击范围内的其他角色并弃置X张与“任”颜色相同的牌，对其造成1点伤害</s></font>；②当你于回合外使用或打出的牌置入弃牌堆时，若此牌与“任”类别相同，你可以令一名角色摸一张牌。（X为你与其体力值之差且至少为1）",
    [":zhenliang2"] = "转换技，①出牌阶段限一次，你可以选择一名攻击范围内的其他角色并弃置X张与“任”颜色相同的牌，对其造成1点伤害；<font color=\"#01A5AF\"><s>②当你于回合外使用或打出的牌置入弃牌堆时，若此牌与“任”类别相同，你可以令一名角色摸一张牌</s></font>。（X为你与其体力值之差且至少为1）",
    ...
    [":duorui"] = "每轮限一次，当你于出牌阶段内对其他角色造成伤害后，你可以废除一个坐骑栏以外的装备栏或两个坐骑栏，然后于此回合内拥有其拥有的“出牌阶段限一次”的技能且拥有其拥有的“每阶段限一次，当你于出牌阶段内”的技能，若如此做，其于其的下回合内这些技能无效。",
    ["$duorui1"] = "夺敌军锐气，杀敌方士气。",
    ["$duorui2"] = "尖锐之势，吾亦可一人夺之。",
    ["zhiti"] = "止啼",
    [":zhiti"] = "锁定技，当你与攻击范围内已受伤的角色拼点赢时或当你因执行【决斗】而对已受伤的你或攻击范围内已受伤的角色造成伤害后，你选择一项：1.恢复一个坐骑栏以外的装备栏；2.恢复两个坐骑栏；锁定技，你攻击范围内已受伤的角色的手牌上限-1；锁定技，若你已受伤，你的手牌上限-1。",
    ["$zhiti1"] = "江东小儿安敢啼哭？",
    ["$zhiti2"] = "娃闻名止啼，孙损十万休！",
    ...
    [":jieyingy"] = "回合开始时，若没有角色拥有“营”标记，你获得1枚“营”标记；结束阶段开始时，你可以弃1枚“营”标记并选择一名其他角色，然后其获得1枚“营”标记；拥有“营”的角色的额定摸牌数+1且手牌上限+1且使用【杀】的次数上限+1；拥有“营”的其他角色的回合结束时，其弃1枚“营”标记，然后你获得其所有手牌。",
    ...
    ["jixu"] = "击虚",
    [":jixu"] = "出牌阶段限一次，你可以选择至少一名体力值相同的其他角色，这些角色各猜你的手牌中是否有【杀】。若其中有角色猜错且你的手牌中：有【杀】，你于此阶段内使用【杀】所有猜错的角色各成为此【杀】的目标；没有【杀】，你弃置所有猜错的角色各一张牌。然后你摸X张牌。若其中没有角色猜错，你结束此阶段。（X为猜错的角色数）",
    ["jixu_yes"] = "有杀",
    ["jixu_no"] = "没杀",
    ["@jixuWrong_replay"] = "猜错",
    ["$jixu1"] = "击虚箭射，懈敌戒备。",
    ...
    ["#liuyao"] = "材匪戡难",
    ["kannan"] = "戡难",
    [":kannan"] = "<font color=\"green\"><b>出牌阶段限X次，</b></font>你可以与你于此阶段内未以此法拼点过的一名角色拼点：赢的角色使用下一张【杀】的伤害值基数+1，然后若其为你，你于于此阶段内不能发动此技能。（X为你的体力值）",
    ...
    ["$wuniang1"] = "集民力万千，亦可为军！",
    ["$wuniang2"] = "集万千一军，定天下大局！",
    ["fangtong"] = "方统",
    [":fangtong"] = "当你的濒死结算完毕后，于此濒死结算内使用【桃】令你回复体力的角色可以失去所有技能，将其的武将牌替换为关索，然后将其的体力上限调整至X，然后你回复1点体力，获得“镇南”。（若其为主公，X为5，否则为4）",
    ["@fangtong"] = "你可以发动“方统”",
    ["$fangtong1"] = "统领方队，为名义所举！",
    ["$fangtong2"] = "三十六方，必为大桶！",
    ["~fangtong"] = "选择一张手牌→选择至少一张“方”→可选步骤：若“方”的点数之和等于36则选择一名其他角色→点击确定",
    ...
    ["#cuiyanmaojie"] = "日出月盛",
    ["zhengpi"] = "征辟",
    [":zhengpi"] = "限定技，出牌阶段，你可以观看军令牌堆顶两张军令牌，然后展示其中一张军令牌，令任意名角色选择是否将势力变成蜀，令所有蜀势力其他角色选择是否执行此军令，若其选择是，其加1点体力上限，回复1点体力。若如此做，你加1点体力上限，回复1点体力，摸X张牌。（X为以此法回复的体力值）",
    ["@zhengpi"] = "你可以选择一张牌",
    ["$zhengpi1"] = "",
    ["$zhengpi2"] = "",
    ["fengying"] = "奉迎",
    ...
    ["zuilun"] = "罪论",
    [":zuilun"] = "结束阶段开始时，你可以观看牌堆顶三张牌，然后获得其中X张牌且可以改变其余牌的顺序，若X为0，你与一名其他角色各失去1点体力。（X为你满足的条件数：1.你于此回合内造成过伤害；2.你于此回合内未弃置过牌；3.你为手牌数最少的角色）",
    ["$zuilun1"] = "吾有三罪，未能除黄皓，制伯约，守国土。",
    ["$zuilun2"] = "数罪当论，吾愧对先帝恩惠。",
    ["fuyin"] = "父荫",
    [":fuyin"] = "锁定技，当你于一名角色的回合内第一次成为【杀】或【决斗】的目标后，若你的手牌数不大于使用者，此牌对你无效。",
    ["$fuyin1"] = "得父荫庇，平步青云。",
    ["$fuyin2"] = "吾自幼心怀父诫，方不愧父亲荫庇。",
...
for i = 0, 10000 do
	local card = sgs.Sanguosha:getEngineCard(i)
	if card == nil then break end
	if not (Set(sgs.Sanguosha:getBanPackages()))[card:getPackage()] and (card:isKindOf("BasicCard") or card:isNDTrick()) and not table.contains(jiaozhao_cards, card:objectName()) and not table.contains(sgs.Sanguosha:getBanPackages(), card:getPackage()) then
...
return {extension, extension_z, extension_pm, extension6, extension7, extension_friend, extension_ol, extension_sp, extension_bf, card_slash, extension_yijiang, extension_god ,extension_hulaoguan, extension_mobile, extension_star, extension_yin, extension_lei}